...
```bash
gh issue create --title "Retries drop the batch silently" --body "The worker retries five times and then drops the batch without logging it, so nobody learns the data is missing."
```
